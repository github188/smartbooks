...
namespace SmartCampus.Utility
{
    ...
    public class ChineseToSpell
    {
        ...
        //参考:    
        //http://blog.chinaunix.net/u/17347/showart.php?id=201191
        private static readonly string[][] chineseDictionary = new string[][]
        {
        ...
        /// <returns></returns>
        public static string Convert(string chinese, int returnType)
        {
            if (chinese == null)
                return null;
            //编码对象
            ...
            //单个汉字的字节数组
            byte[] chineseBytes = new byte[2];
            //返回的拼音或简拼
            StringBuilder returnSpell = new StringBuilder();
            //对汉字字符数组进行遍历
            for (int i = 0; i < chineseChars.Length; i++)
            {
                //获得汉字字符的字节编码
                chineseBytes = code.GetBytes(chineseChars[i].ToString());
                //字母或数字
                if (chineseBytes.Length == 1)
                {
                    returnSpell.Append(chineseChars[i]);
                    continue;
                }
                //得到区域
                areaNo = chineseBytes[0] - 0xA0;
                //是gb2312汉字
                if (areaNo >= 0x10 && areaNo <= 0x57)
                {
                    //是否发现汉字
                    bool isFind = false;
                    //汉字拼音
                    string tempSell = "";
                    //遍历汉字字典查找匹配汉字
                    for (int j = 0; j < chineseDictionary.Length; j++)
                    {
                        if (chineseDictionary[j][1].IndexOf(chineseChars[i]) != -1)
                        {
                            tempSell = chineseDictionary[j][0];
                            //要返回简拼
                            if (returnType != 0)
                            {
                                //是卷舌音，取前两位
                                if (tempSell.IndexOf('h') > 0)
                                {
                                    tempSell = tempSell.Substring(0, 2).ToUpper();
                                }
                                //其它取前一位
                                else
                                {
                                    tempSell = tempSell.Substring(0, 1).ToUpper();
                                }
                            }
                            returnSpell.Append(tempSell);
                            isFind = true;
                            break;
                        }
                    }
