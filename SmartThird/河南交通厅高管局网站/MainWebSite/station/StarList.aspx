<%@ Page Language="C#" AutoEventWireup="true" CodeFile="StarList.aspx.cs" Inherits="station_StarList" %>
...
<head id="Head1" runat="server">
    ...
    <title><%=((DataTable)ViewState["dtType"]).Rows[0]["T_Name"]%>-收费管理-河南省交通运输厅高速公路管理局门户网站</title>
...
<body>
  ...
  <div class="right wid580">
    	<div class="weizhi">您的位置：<a href="../index.aspx">首页</a> >> <a href="Station_Index.aspx">收费管理</a> >> <%=((DataTable)ViewState["dtType"]).Rows[0]["T_Name"]%></div>
        <ul class="alllist">
            <asp:Repeater ID="rptNewsList" runat="server">
              <ItemTemplate>
                <li><span>[<%# PubClass.Tool.Get_ShortDate(Eval("N_Time").ToString())%>]</span><a href='StarInfo.aspx?nid=<%#  Eval("N_ID") %>' target="_blank" title='<%# Eval("N_Title") %>'><%# PubClass.Tool.SubString("["+Eval("N_From")+"]" + Eval("N_Title").ToString(), 37)%> </a></li>
              </ItemTemplate>
            </asp:Repeater>  
        </ul>
        <div class="fanye">
