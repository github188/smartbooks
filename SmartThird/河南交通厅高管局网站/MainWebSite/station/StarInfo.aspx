<%@ Page Language="C#" AutoEventWireup="true" CodeFile="StarInfo.aspx.cs" Inherits="station_StarInfo" %>
...
<head id="Head1" runat="server">
    ...
    <title><%=((DataTable)ViewState["dtNews"]).Rows[0]["N_Title"] + "-" + ((DataTable)ViewState["dtNews"]).Rows[0]["T_Name"] %>-收费管理-河南省交通运输厅高速公路管理局门户网站</title>
...
<body>
  ...
  <div class="right wid580">
    <div class="weizhi">您的位置：<a href="../index.aspx">首页</a> >> <a href="Station_Index.aspx">收费管理</a> >> <a href="StarList.aspx?tid=<%=((DataTable)ViewState["dtNews"]).Rows[0]["T_ID"]%>"><%=((DataTable)ViewState["dtNews"]).Rows[0]["T_Name"]%></a></div>
    	<div class="pages">
        		<h1><%=((DataTable)ViewState["dtNews"]).Rows[0]["N_Title"]%></h1>
                <p class="page_date">服务之星：<%=((DataTable)ViewState["dtNews"]).Rows[0]["N_From"]%>  &nbsp;&nbsp;发布日期：<%=PubClass.Tool.Get_ShortDate(((DataTable)ViewState["dtNews"]).Rows[0]["N_Time"].ToString())%> </p>
                <div class="page_con">
                   <%=((DataTable)ViewState["dtNews"]).Rows[0]["N_Content"]%>
                    <%
                        if (((DataTable)ViewState["dtNews"]).Rows[0]["N_ImgPath"] != null && ((DataTable)ViewState["dtNews"]).Rows[0]["N_ImgPath"].ToString() != "") { 
                        %>
                          <div style="padding:3px; text-align:center;"><img src='../newsimages/<%=((DataTable)ViewState["dtNews"]).Rows[0]["N_ImgPath"]%>'  alt='<%=((DataTable)ViewState["dtNews"]).Rows[0]["N_Title"]%>'/></div>
                        <%
                        }
                         %>
                </div>
