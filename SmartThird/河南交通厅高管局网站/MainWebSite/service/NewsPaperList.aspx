<%@ Page Language="C#" AutoEventWireup="true" CodeFile="NewsPaperList.aspx.cs" Inherits="service_NewsPaperList" %>
...
<head runat="server">
    ...
    <title>服务区行业报刊-服务区管理-河南省交通运输厅高速公路管理局门户网站</title>
...
<body>
 ...
 <div style=" width:800px; margin:0 auto;"><iframe src="../CommonTop.aspx" scrolling="no" frameborder="0" style=" margin:0 auto; width:800px; height:253px; overflow:hidden;"></iframe></div>
  ...
  <div class="right wid580">
    <div class="weizhi">您的位置：<a href="../index.aspx">首页</a> >> <a href="Service_Index.aspx">服务区管理</a> >> 行业报刊</div>
    	<div class="pages"> 
            <ul id="covers">
                ...
                <div class="paperImage">
                    <div class="outer"><a  href='ViewNewsPaper.aspx?nid=<%# Eval("N_ID") %>' target="_blank"  title='<%# Eval("N_Title") %>'><img src='../newspaper/<%# Eval("N_AImgView") %>' class="pageSmall" alt='<%# Eval("N_Title") %>'/></a></div>
                    <a href='ViewNewsPaper.aspx?nid=<%# Eval("N_ID") %>' target="_blank"  title='<%# Eval("N_Title") %>'><%# Eval("N_Title") %></a>
                    <div class="p_txt">出版日期:<%#PubClass.Tool.Get_ShortDate(Eval("N_Time").ToString()) %></div>
                </div>
            </li>
                </ItemTemplate>
                </asp:Repeater>
        </ul>
            <div class="fanye"><webdiyer:AspNetPager ID="AspNetPager1" runat="server" CustomInfoHTML="第%CurrentPageIndex%页，共%PageCount%页，每页%PageSize%条" FirstPageText="首页" LastPageText="尾页" NextPageText="下一页" PageIndexBoxType="DropDownList" PageSize="20" PrevPageText="上一页" ShowCustomInfoSection="Left" ShowPageIndexBox="Always" SubmitButtonText="Go" TextAfterPageIndexBox="页" TextBeforePageIndexBox="转到" OnPageChanged="AspNetPager1_PageChanged">
