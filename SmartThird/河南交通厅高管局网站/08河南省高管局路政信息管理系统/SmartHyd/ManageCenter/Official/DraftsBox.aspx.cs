...
namespace SmartHyd.ManageCenter.Official {
    public partial class DraftsBox : System.Web.UI.Page {
        ...
        protected void grvAcceptList_RowCommand(object sender, GridViewCommandEventArgs e) {
            ...
            switch (e.CommandName) {
                /*跳转到回复页面*/
                case "edit":
                    string url = string.Format("~/ManageCenter/Official/Create.aspx?id={0}&m=edit", id.ToString());
        ...
        //绑定草稿箱列表数据
        private void BindDraftsBoxList() {
            ...
            /*获取当前用户的草稿数据*/
            dt = bll.GetList(string.Format("USERID={0} AND STATUS=2 ORDER BY TIMESTAMP DESC", userSession.USERID.ToString()));
